// ---------------------------------------------------------------------------
// Review Request Email — DAHAB Transactional Email
// Sent when order status changes to LIVREE
// ---------------------------------------------------------------------------

export interface ReviewRequestData {
  orderNumber: string;
  customerName: string;
  /** Slug or full URL of the first product in the order for review link */
  productSlug?: string;
}

const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://dahab.ma";

export function generateReviewRequestEmail(order: ReviewRequestData): {
  subject: string;
  html: string;
} {
  const reviewUrl = order.productSlug
    ? `${SITE_URL}/produit/${order.productSlug}#avis`
    : `${SITE_URL}/boutique`;

  const subject = `Comment avez-vous trouve votre bijou ? — DAHAB`;

  const html = `<!DOCTYPE html>
<html lang="fr">
<head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>Donnez votre avis DAHAB</title></head>
...
<tr>
  <td style="padding:40px 32px;text-align:center;">
    ...
    <h1 style="font-family:Georgia,'Times New Roman',serif;font-size:24px;color:#1A1A1A;margin:0 0 8px;">Comment avez-vous trouve votre bijou ?</h1>
    ...
    <p style="font-family:'Inter',Arial,sans-serif;font-size:15px;color:#555;line-height:1.6;margin:0 0 24px;">
      Nous esperons que votre commande <strong style="color:#1A1A1A;">${order.orderNumber}</strong> vous plait ! Votre avis nous aide a continuer a creer de beaux bijoux pour vous.
    </p>

    <!-- Incentive box -->
    <div style="background-color:#FAF7F2;border-radius:8px;padding:24px;margin:0 0 28px;">
      <p style="font-family:'Inter',Arial,sans-serif;font-size:13px;color:#555;margin:0 0 12px;">
        Laissez un avis et recevez :
      </p>
      <p style="font-family:Georgia,'Times New Roman',serif;font-size:28px;color:#C9A84C;font-weight:bold;margin:0 0 4px;">-5%</p>
      <p style="font-family:'Inter',Arial,sans-serif;font-size:14px;color:#1A1A1A;margin:0;">
        sur votre prochaine commande
      </p>
      <p style="font-family:'Inter',Arial,sans-serif;font-size:12px;color:#888;margin:8px 0 0;">
        Code promo envoye apres publication de votre avis
      </p>
    </div>
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="margin:0 auto 24px;">
      <tr>
        <td style="background-color:#C9A84C;border-radius:6px;padding:14px 32px;text-align:center;">
          <a href="${reviewUrl}" style="color:#ffffff;font-family:'Inter',Arial,sans-serif;font-size:14px;font-weight:600;text-decoration:none;display:inline-block;">Laisser un Avis</a>
        </td>
      </tr>
    </table>

    <p style="font-family:'Inter',Arial,sans-serif;font-size:13px;color:#888;line-height:1.6;margin:0 0 16px;">
      Cela ne prend que 30 secondes et aide d'autres clientes a faire leur choix.
    </p>

    <p style="font-family:'Inter',Arial,sans-serif;font-size:13px;color:#888;margin:0;">
      Une question ? Contactez-nous sur <a href="https://wa.me/212600000000" style="color:#C9A84C;text-decoration:none;font-weight:600;">WhatsApp</a>
    </p>
  </td>
</tr>
